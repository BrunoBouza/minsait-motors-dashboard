...
def create_seasonal_acf_pacf_plot(y, seasonal_period=52, max_lags=40):
    """
    Crea gráficas ACF y PACF para datos con diferenciación estacional.
    Útil para identificar parámetros P y Q en modelos SARIMA.
    
    Args:
        y (np.array): Serie temporal de ventas
        seasonal_period (int): Período estacional (ej: 52 para datos semanales anuales)
        max_lags (int): Número máximo de lags a mostrar
        
    Returns:
        plotly.graph_objects.Figure: Figura con ACF y PACF de datos diferenciados
    """
    # Calcular diferencia estacional: y_t - y_(t-s)
    if len(y) <= seasonal_period:
        raise ValueError(f"No hay suficientes datos para diferencia estacional de período {seasonal_period}")
    
    seasonal_diff = y[seasonal_period:] - y[:-seasonal_period]
    
    # Limitar max_lags según el tamaño de los datos diferenciados
    max_lags = min(max_lags, len(seasonal_diff) // 2 - 1)
    
    # Calcular ACF y PACF de la serie diferenciada
    acf_values = acf(seasonal_diff, nlags=max_lags, fft=False)
    pacf_values = pacf(seasonal_diff, nlags=max_lags, method='ywm')
    
    # Calcular intervalos de confianza (95%)
    confidence_interval = 1.96 / np.sqrt(len(seasonal_diff))
    
    # Crear subplots
    fig = make_subplots(
        rows=2, cols=1,
        subplot_titles=(
            f'ACF - Diferencia Estacional (período={seasonal_period})', 
            f'PACF - Diferencia Estacional (período={seasonal_period})'
        ),
        vertical_spacing=0.12
    )
    ...
    fig.update_layout(
        height=700,
        template='plotly_white',
        title_text=f"ACF/PACF de Diferencia Estacional (y_t - y_{{t-{seasonal_period}}})",
        title_x=0.5,
        showlegend=False
    )
